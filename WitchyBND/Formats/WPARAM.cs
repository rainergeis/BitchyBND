...
internal static class WPARAM
{
    ...
    public static bool Unpack(this PARAM param, string sourceFile, string sourceDir, WBUtil.GameType game)
    {
        ...
        var paramName = Path.GetFileNameWithoutExtension(sourceFile);

        // Begin writing the XML
        var xws = new XmlWriterSettings();
        xws.Indent = true;
        var xw = XmlWriter.Create(Path.Combine(sourceDir, $"{sourceFile}.xml"), xws);
        ...
        IEnumerable<string> fieldNames = paramdef.Fields.Select(field => {
        ...
        foreach (PARAM.Row row in param.Rows)
        {
            ...
            foreach (var fieldName in fieldNames)
            {
                ...
                if (cell == null) throw new Exception($"Row {id} is missing cell {fieldName}.");
        ...
        foreach (KeyValuePair<string, List<string>> pair in allValues)
        {
            ...
            if (maxValue.Count >= pair.Value.Count() * 0.8) defaultValues[fieldName] = maxValue.Key;
        }
        ...
        foreach (PARAMDEF.Field field in paramdef.Fields)
        {
            ...
            // commonFieldValues[field.InternalName] = maxValue.Key;
            // }
            if (defaultValues.ContainsKey(fieldName)) xw.WriteAttributeString("defaultValue", defaultValues[fieldName]);
        ...
        foreach (WPARAMRow row in rows)
        {
            ...
            else if (row.ParamdexName != null) xw.WriteAttributeString("paramdexName", row.ParamdexName);
            ...
            var csvValues = new List<string>();
            {
                foreach (KeyValuePair<string, string> fieldPair in row.Fields)
                {
                    ...
                    var value = fieldPair.Value;

                    var isDefaultValue = defaultValues.ContainsKey(fieldName) &&
    ...
    public static bool Repack(string sourceFile, string sourceDir)
    {
        ...
        foreach (XmlNode xmlRow in xml.SelectNodes("param/rows/row"))
        {
            ...
            {
                var fieldName = field.InternalName;
                var defaultValue = defaultValues[field.InternalName];
                var value = defaultValue;
                switch (cellStyle)
                {
                    case CellStyle.Element:
                        XmlNode fieldNode = xmlRow.SelectSingleNode(fieldName);
                        if (fieldNode != null) value = fieldNode.InnerText;

                        break;
                    case CellStyle.Attribute:
                        XmlAttribute attribute = xmlRow.Attributes[fieldName];
                        if (attribute != null) value = attribute.InnerText;

                        break;
                    case CellStyle.CSV:
                        var fieldIdx = paramdef.Fields.IndexOf(field);
                        if (csv != null && csv.Length > fieldIdx) value = csv[fieldIdx];

                        break;
                }

                if (value == null) throw new Exception($"Row {id} is missing value for cell {fieldName}.");
        ...
        try
        {
            ...
            if (!param.ApplyParamdefLessCarefully(paramdef)) throw new Exception("Paramdef did not apply carefully");
        }
        catch (Exception e)
        {
        ...
        var outPath = Path.Combine(sourceDir, sourceFile.Replace(".param.xml", ".param"));
    ...
    public static void PopulateParamdef(WBUtil.GameType game)
    {
        ...
        if (!Directory.Exists(paramdefPath)) throw new Exception($"Paramdef path not found for {gameName}.");

        IOrderedEnumerable<string> patchParamdefPaths = Directory
            .GetDirectories($@"{WBUtil.GetExeLocation()}\Assets\Paramdex\{gameName}\DefsPatch")
            .OrderByDescending(path => path);

        // Reading XML paramdefs
        foreach (var path in Directory.GetFiles(paramdefPath, "*.xml"))
        {
            var paramdef = PARAMDEF.XmlDeserialize(path);

            foreach (var patchPath in patchParamdefPaths)
            {
            ...
            foreach (PARAMDEF.Field dupe in pair.Value)
    ...
    public static void PopulateNames(WBUtil.GameType game, string paramName)
    {
        ...
        foreach (var name in names)
        {
            ...
            try
            {
                ...
                if (result == false) Console.WriteLine($"Paramdex: Duplicate name for ID {splitted[0]}");
            }
            catch (Exception e)
            {
    ...
    public static object StringToCellValue(string valueString, PARAMDEF.Field pdefField)
    {
        ...
        {
            IEnumerable<byte> bytes = valueString.Substring(0, valueString.Length - 1).Substring(1).Split('|')
