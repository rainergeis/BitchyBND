...
internal class Program
{
    ...
    private static WBUtil.GameType? game;
    ...
    private static void Main(string[] args)
    {
        ...
        foreach (var arg in args)
            if (arg.Contains('*'))
            {
                ...
                PatternMatchingResult match = matcher.Match(Path.Combine(Environment.CurrentDirectory, root), files);
                if (match.HasMatches) paths.AddRange(match.Files.Select(m => Path.Combine(root, m.Path)).ToList());
            }
            else
            {
                paths.Add(arg);
        ...
        foreach (var halfPath in paths)
        {
            try
            {
                var path = Path.GetFullPath(halfPath);
    ...
    {
        var sourceDir = new FileInfo(sourceFile).Directory.FullName;
        ...
        if (fileName.Contains("regulation.bnd.dcx") || fileName.Contains("Data0") ||
            fileName.Contains("regulation.bin") ||
        ...
        if (DCX.Is(sourceFile))
        {
            ...
            var bytes = TryDecompressBytes(sourceFile, out DCX.Type compr);
            compression = compr;
            ...
            else if (FFXDLSE.Is(bytes))
            {
                ...
                FFXDLSE ffx = FFXDLSE.Read(bytes);
        ...
        else
        {
            ...
            {
                Console.WriteLine($"Unpacking FFX: {fileName}...");
                FFXDLSE ffx = FFXDLSE.Read(sourceFile);
                ffx.Unpack(sourceFile);
            }
            ...
            {
                if (game == null) game = WBUtil.DetermineParamdexGame(sourceDir);
...
###############BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB#########");
    ...
    private static byte[] TryDecompressBytes(string sourceFile, out DCX.Type compression)
    {
        ...
        {
            var oo2corePath = WBUtil.GetOodlePath();
            if (oo2corePath == null)
                throw;

            var handle = Kernel32.LoadLibrary(oo2corePath);
            var bytes = DCX.Decompress(sourceFile, out compression);
        ...
        IProgress<float> progress)
    {
        ...
        {
            var destPath = Path.Combine(sourceDir, "Data0.bdt");
        ...
        if (fileName.Contains("enc_regulation.bnd.dcx"))
        {
            var destPath = Path.Combine(sourceDir, fileName);
            BND4 bnd;
            if (!BND4.IsRead(destPath, out bnd)) bnd = WBUtil.DecryptDS2Regulation(destPath);
        ...
        IProgress<float> progress)
    {
        ...
        var regFile = $"{targetDir}\\{filename}";
        ...
        if (filename.Contains("enc_regulation.bnd.dcx"))
        {
            ...
            var destPath = Path.Combine(sourceDir, sourceName);
            BND4
                bnd = BND4.Read(
    ...
    private static bool RepackDir(string sourceDir, IProgress<float> progress)
    {
        ...
        foreach (var dir in Directory.EnumerateDirectories(sourceDir)) Repacked = RepackDir(dir, progress) || Repacked;
