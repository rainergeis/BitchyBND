...
public class BXF4 : IBinder, IBXF4
{
    ...
    internal static List<BinderFileHeader> ReadBHFHeader(IBXF4 bxf, BinaryReaderEx br)
    {
        ...
        var fileHeaderSize = br.ReadInt64();
        br.AssertInt64(0);
        ...
        if (fileHeaderSize != Binder.GetBND4FileHeaderSize(bxf.Format))
            throw new FormatException(
                $"File header size for format {bxf.Format} is expected to be 0x{Binder.GetBND4FileHeaderSize(bxf.Format):X}, but was 0x{fileHeaderSize:X}");

        br.AssertInt32(0);

        if (bxf.Extended == 4)
        {
            var hashGroupsOffset = br.ReadInt64();
