...
public class BinderFileHeader
{
    ...
    private BinderFileHeader(FileFlags flags, int id, string name, long compressedSize, long uncompressedSize,
        long dataOffset)
    {
        Flags = flags;
        ID = id;
    ...
    /// <summary>
    ///     Size of the file after compression (or just the size of the file, if not compressed). Do not modify unless you know
    ///     what you're doing.
    /// </summary>
    ...
    internal static BinderFileHeader ReadBinder4FileHeader(BinaryReaderEx br, Format format, bool bitBigEndian,
        bool unicode)
    {
        FileFlags flags = ReadFileFlags(br, bitBigEndian);
        br.AssertByte(0);
    ...
    private void WriteFileData(BinaryWriterEx bw, byte[] bytes)
    {
        ...
        if (IsCompressed(Flags))
        {
            var compressed = DCX.Compress(bytes, CompressionType);
    ...
    internal void WriteBinder3FileData(BinaryWriterEx bwHeader, BinaryWriterEx bwData, Format format, int index,
        byte[] bytes)
    {
    ...
    internal void WriteBinder4FileData(BinaryWriterEx bwHeader, BinaryWriterEx bwData, Format format, int index,
        byte[] bytes)
    {
        WriteFileData(bwData, bytes);
