...
public class MCG : SoulsFile<MCG>
{
    ...
    protected override void Read(BinaryReaderEx br)
    {
        ...
        var nodesOffset = br.ReadInt32();
        var edgeCount = br.ReadInt32();
        var edgesOffset = br.ReadInt32();
        Unk18 = br.ReadInt32();
        Unk1C = br.ReadInt32();
    ...
    public override bool Validate(out Exception ex)
    {
        ...
        for (var i = 0; i < Nodes.Count; i++)
        {
            ...
            if (!ValidateNull(node, $"{nameof(Nodes)}[{i}]: {nameof(Node)} may not be null.", out ex)
                || !ValidateNull(node.ConnectedNodeIndices,
                    $"{nameof(Nodes)}[{i}]: {nameof(Node.ConnectedNodeIndices)} may not be null.", out ex)
                || !ValidateNull(node.ConnectedEdgeIndices,
                    $"{nameof(Nodes)}[{i}]: {nameof(Node.ConnectedEdgeIndices)} may not be null.", out ex))
                return false;

            if (node.ConnectedNodeIndices.Count != node.ConnectedEdgeIndices.Count)
            {
                ex = new InvalidDataException(
                    $"{nameof(Nodes)}[{i}]: {nameof(Node.ConnectedNodeIndices)} count must equal {nameof(Node.ConnectedEdgeIndices)} count.");
                return false;
            }

            for (var j = 0; j < node.ConnectedNodeIndices.Count; j++)
            {
                var nodeIndex = node.ConnectedNodeIndices[j];
                var edgeIndex = node.ConnectedEdgeIndices[j];
                if (!ValidateIndex(Nodes.Count, nodeIndex,
                        $"{nameof(Nodes)}[{i}].{nameof(Node.ConnectedNodeIndices)}[{j}]: Index out of range: {nodeIndex}",
                        out ex)
                    || !ValidateIndex(Edges.Count, edgeIndex,
                        $"{nameof(Nodes)}[{i}].{nameof(Node.ConnectedEdgeIndices)}[{j}]: Index out of range: {edgeIndex}",
                        out ex))
                    return false;
            }
        }
        ...
        for (var i = 0; i < Edges.Count; i++)
        {
            ...
            if (!ValidateNull(edge, $"{nameof(Edges)}[{i}]: {nameof(Edge)} may not be null.", out ex)
                || !ValidateNull(edge.UnkIndicesA, $"{nameof(Edges)}[{i}]: {nameof(Edge.UnkIndicesA)} may not be null.",
                    out ex)
                || !ValidateNull(edge.UnkIndicesB, $"{nameof(Edges)}[{i}]: {nameof(Edge.UnkIndicesB)} may not be null.",
                    out ex)
                || !ValidateIndex(Nodes.Count, edge.NodeIndexA,
                    $"{nameof(Edges)}[{i}].{nameof(Edge.NodeIndexA)}: Index out of range: {edge.NodeIndexA}", out ex)
                || !ValidateIndex(Nodes.Count, edge.NodeIndexB,
                    $"{nameof(Edges)}[{i}].{nameof(Edge.NodeIndexB)}: Index out of range: {edge.NodeIndexB}", out ex))
                return false;
        }
    ...
    public class Node
    {
        ...
        {
            var connectionCount = br.ReadInt32();
            Position = br.ReadVector3();
            var nodeIndicesOffset = br.ReadInt32();
            var edgeIndicesOffset = br.ReadInt32();
            Unk18 = br.ReadInt32();
            Unk1C = br.ReadInt32();
    ...
    public class Edge
    {
        ...
        internal Edge(BinaryReaderEx br)
        {
            ...
            var indexCountA = br.ReadInt32();
            var indicesOffsetA = br.ReadInt32();
            NodeIndexB = br.ReadInt32();
            var indexCountB = br.ReadInt32();
            var indicesOffsetB = br.ReadInt32();
