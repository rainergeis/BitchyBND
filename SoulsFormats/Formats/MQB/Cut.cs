...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member
    public class Cut
    {
        ...
        internal Cut(BinaryReaderEx br, MQBVersion version)
        {
            ...
            var disposCount = br.ReadInt32();
            Unk44 = br.ReadInt32();
            Duration = br.ReadInt32();
            br.AssertInt32(0);

            var timelineCount = br.ReadInt32();
            if (version == MQBVersion.DarkSouls2Scholar)
                br.AssertInt32(0);
            var timelinesOffset = br.ReadVarint();
            if (version != MQBVersion.DarkSouls2Scholar)
                br.AssertInt64(0);

            var disposesByOffset = new Dictionary<long, Disposition>(disposCount);
            for (var i = 0; i < disposCount; i++)
        ...
        internal void Write(BinaryWriterEx bw, MQBVersion version, Dictionary<Disposition, long> offsetsByDispos,
            int cutIndex, List<CustomData> allCustomData, List<long> customDataValueOffsets)
        {
            var disposCount = Timelines.Sum(g => g.Dispositions.Count);
            bw.WriteFixStrW(Name, 0x40);
        ...
        internal void WriteTimelineCustomData(BinaryWriterEx bw, int cutIndex, List<CustomData> allCustomData,
            List<long> customDataValueOffsets)
        {
            for (var i = 0; i < Timelines.Count; i++)
