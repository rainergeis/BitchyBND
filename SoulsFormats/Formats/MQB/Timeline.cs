...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member
    public class Timeline
    {
        ...
        {
            var disposOffsetsOffset = br.ReadVarint();
            var disposCount = br.ReadInt32();
            if (version == MQBVersion.DarkSouls2Scholar)
                br.AssertInt32(0);
            var customDataOffset = br.ReadVarint();
            var customDataCount = br.ReadInt32();
            Unk10 = br.ReadInt32();

            Dispositions = new List<Disposition>(disposCount);
            var disposOffsets = br.GetVarints(disposOffsetsOffset, disposCount);
            foreach (var disposOffset in disposOffsets)
            {
            ...
            br.StepIn(customDataOffset);
            {
                ...
                for (var i = 0; i < customDataCount; i++)
        ...
        internal void WriteDispositions(BinaryWriterEx bw, Dictionary<Disposition, long> offsetsByDispos,
            List<CustomData> allCustomData, List<long> customDataValueOffsets)
        {
        ...
        internal void WriteCustomData(BinaryWriterEx bw, int cutIndex, int timelineIndex,
            List<CustomData> allCustomData, List<long> customDataValueOffsets)
        {
