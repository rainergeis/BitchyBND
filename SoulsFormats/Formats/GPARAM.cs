...
public class GPARAM : SoulsFile<GPARAM>
{
    ...
    protected override void Read(BinaryReaderEx br)
    {
        ...
        if (Game == GPGame.Sekiro) Unk50 = br.ReadSingle();
        ...
        for (var i = 0; i < unk3Count; i++)
            Unk3s.Add(new Unk3(br, Game, offsets));

        if (Game == GPGame.DarkSouls3 || Game == GPGame.Sekiro)
        {
            var commentOffsetsOffsets = br.GetInt32s(offsets.CommentOffsetsOffsets, groupCount);
            var commentOffsetsLength = offsets.Comments - offsets.CommentOffsets;
            for (var i = 0; i < groupCount; i++)
            {
                int commentCount;
                if (i == groupCount - 1)
                ...
                {
                    var commentOffset = br.ReadInt32();
                    var comment = br.GetUTF16(offsets.Comments + commentOffset);
    ...
    protected override void Write(BinaryWriterEx bw)
    {
        ...
        if (Game == GPGame.Sekiro) bw.WriteSingle(Unk50);
        ...
        for (var i = 0; i < Unk3s.Count; i++)
            Unk3s[i].WriteHeader(bw, Game, i);

        var unk3ValuesOffset = (int)bw.Position;
    ...
    public class Group
    {
        ...
        {
            var groupHeaderOffset = br.ReadInt32();
            br.StepIn(offsets.GroupHeaders + groupHeaderOffset);
            {
                var paramCount = br.ReadInt32();
                var paramHeaderOffsetsOffset = br.ReadInt32();
                if (game == GPGame.DarkSouls2)
                {
    ...
    public class Param
    {
        ...
        {
            var paramHeaderOffset = br.ReadInt32();
            br.StepIn(offsets.ParamHeaders + paramHeaderOffset);
            {
                var valuesOffset = br.ReadInt32();
                var valueIDsOffset = br.ReadInt32();

                Type = br.ReadEnum8<ParamType>();
                var valueCount = br.ReadByte();
                br.AssertByte(0);
                br.AssertByte(0);
        ...
        internal void WriteParamHeader(BinaryWriterEx bw, GPGame game, int groupIndex, int paramIndex,
    ...
    public class Unk3
    {
        ...
        internal Unk3(BinaryReaderEx br, GPGame game, Offsets offsets)
        {
            ...
            var valueIDsOffset = br.ReadUInt32();
            if (game == GPGame.Sekiro)
                Unk0C = br.ReadInt32();
