...
public partial class DRB
{
    ...
    private void Read(BinaryReaderEx br, DRBVersion version)
    {
        ...
        var shprStart = ReadBlobBlock(br, "SHPR");
        var ctprStart = ReadBlobBlock(br, "CTPR");
        AnipBytes = ReadBlobBytes(br, "ANIP");
        IntpBytes = ReadBlobBytes(br, "INTP");
        var scdpStart = ReadBlobBlock(br, "SCDP");
            ...
            if (shape is Shape.Dialog dialog)
            {
                if (dialog.Dlg == null)
                    ...
                    throw new InvalidDataException(
                        $"Dlg \"{dialog.Dlg.Name}\" is referenced but not found in Dlgs list.");
            }
        }
    ...
    {
        var start = ReadBlockHeader(br, "STR\0", out var count, out var size);
        var strings = new Dictionary<int, string>(count);
        for (var i = 0; i < count; i++)
        {
            var offset = (int)(br.Position - start);
    ...
    {
        var start = WriteBlockHeader(bw, "STR\0");
        ...
        foreach (Scdl scdl in Scdls)
        {
            ...
            foreach (Scdo scdo in scdl.Scdos)
            {
                ...
                foreach (Scdk scdk in scdo.Scdks) writeString(scdk.Name);
            }
        }
    ...
    {
        var start = ReadBlockHeader(br, "TEXI", out var count, out var size);
        var textures = new List<Texture>(count);
        for (var i = 0; i < count; i++) textures.Add(new Texture(br, strings));
        br.Position = start + size;
        return textures;
    }
    ...
    {
        var start = WriteBlockHeader(bw, "TEXI");
        foreach (Texture tex in Textures)
    ...
    {
        var start = WriteBlobBlock(bw, "SHPR");
        var shprOffsets = new Queue<int>();

        void writeShape(Shape shape)
        {
            var offset = (int)(bw.Position - start);
        ...
        foreach (Dlg dlg in Dlgs)
        {
            ...
            foreach (Dlgo dlgo in dlg.Dlgos) writeShape(dlgo.Shape);
        }
    ...
    {
        var start = WriteBlobBlock(bw, "CTPR");
        ...
        foreach (Dlg dlg in Dlgs)
        {
            ...
            foreach (Dlgo dlgo in dlg.Dlgos) writeControl(dlgo.Control);
        }
    ...
    {
        var start = WriteBlobBlock(bw, "SCDP");
    ...
    private Dictionary<int, Shape> ReadSHAP(BinaryReaderEx br, DRBVersion version, Dictionary<int, string> strings,
        long shprStart)
    {
        var start = ReadBlockHeader(br, "SHAP", out var count, out var size);
    ...
    private Queue<int> WriteSHAP(BinaryWriterEx bw, Dictionary<string, int> stringOffsets, Queue<int> shprOffsets,
        out Queue<int> dlgoShapOffsets)
    {
        var start = WriteBlockHeader(bw, "SHAP");
        ...
        foreach (Dlg dlg in Dlgs)
        {
            ...
            foreach (Dlgo dlgo in dlg.Dlgos) dlgoShapOffsets.Enqueue(writeShape(dlgo.Shape));
        }
    ...
    {
        var start = ReadBlockHeader(br, "CTRL", out var count, out var size);
    ...
    private Queue<int> WriteCTRL(BinaryWriterEx bw, Dictionary<string, int> stringOffsets, Queue<int> ctprOffsets,
        out Queue<int> dlgoCtrlOffsets)
    {
        var start = WriteBlockHeader(bw, "CTRL");
        ...
        foreach (Dlg dlg in Dlgs)
        {
            ...
            foreach (Dlgo dlgo in dlg.Dlgos) dlgoCtrlOffsets.Enqueue(writeControl(dlgo.Control));
        }
    ...
    {
        var start = ReadBlockHeader(br, "ANIK", out var count, out var size);
    ...
    {
        var start = WriteBlockHeader(bw, "ANIK");
        var count = 0;
    ...
    {
        var start = ReadBlockHeader(br, "ANIO", out var count, out var size);
    ...
    {
        var start = WriteBlockHeader(bw, "ANIO");
        var count = 0;
    ...
    {
        var start = ReadBlockHeader(br, "ANIM", out var count, out var size);
        var anims = new List<Anim>(count);
        for (var i = 0; i < count; i++) anims.Add(new Anim(br, strings, anios));
        br.Position = start + size;
        return anims;
    }
    ...
    {
        var start = WriteBlockHeader(bw, "ANIM");
        foreach (Anim anim in Anims) anim.Write(bw, stringOffsets, anioOffsets);
    ...
    {
        var start = ReadBlockHeader(br, "SCDK", out var count, out var size);
    ...
    {
        var start = WriteBlockHeader(bw, "SCDK");
        var count = 0;
    ...
    private Dictionary<int, Scdo> ReadSCDO(BinaryReaderEx br, Dictionary<int, string> strings,
        Dictionary<int, Scdk> scdks)
    {
        var start = ReadBlockHeader(br, "SCDO", out var count, out var size);
    ...
    {
        var start = WriteBlockHeader(bw, "SCDO");
        var count = 0;
    ...
    {
        var start = ReadBlockHeader(br, "SCDL", out var count, out var size);
        var scdls = new List<Scdl>(count);
        for (var i = 0; i < count; i++) scdls.Add(new Scdl(br, strings, scdos));
        br.Position = start + size;
        return scdls;
    }
    ...
    {
        var start = WriteBlockHeader(bw, "SCDL");
        foreach (Scdl scdl in Scdls) scdl.Write(bw, stringOffsets, scdoOffsets);
        FinishBlockHeader(bw, "SCDL", start, Scdls.Count);
    }

    private Dictionary<int, Dlgo> ReadDLGO(BinaryReaderEx br, Dictionary<int, string> strings,
        Dictionary<int, Shape> shapes, Dictionary<int, Control> controls)
    {
        var start = ReadBlockHeader(br, "DLGO", out var count, out var size);
    ...
    private Queue<int> WriteDLGO(BinaryWriterEx bw, Dictionary<string, int> stringOffsets, Queue<int> shapOffsets,
        Queue<int> ctrlOffsets)
    {
        var start = WriteBlockHeader(bw, "DLGO");
        var count = 0;
    ...
    private List<Dlg> ReadDLG(BinaryReaderEx br, Dictionary<int, string> strings, Dictionary<int, Shape> shapes,
        Dictionary<int, Control> controls, Dictionary<int, Dlgo> dlgos)
    {
        var start = ReadBlockHeader(br, "DLG\0", out var count, out var size);
        var dlgs = new List<Dlg>(count);
        for (var i = 0; i < count; i++) dlgs.Add(new Dlg(br, strings, shapes, controls, dlgos));
        br.Position = start + size;
        return dlgs;
    }

    private void WriteDLG(BinaryWriterEx bw, Dictionary<string, int> stringOffsets, Queue<int> shapOffsets,
        Queue<int> ctrlOffsets, Queue<int> dlgoOffsets)
    {
        var start = WriteBlockHeader(bw, "DLG\0");
        foreach (Dlg dlg in Dlgs) dlg.Write(bw, stringOffsets, shapOffsets, ctrlOffsets, dlgoOffsets);
    ...
    {
        var value = color.A | ((uint)color.R << 24) | ((uint)color.G << 16) | ((uint)color.B << 8);
