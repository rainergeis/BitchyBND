...
public class RMB : SoulsFile<RMB>
{
    ...
    public override bool Validate(out Exception ex)
    {
        ...
        for (var i = 0; i < Rumbles.Count; i++)
        {
            Rumble rumble = Rumbles[i];
            if (!ValidateNull(rumble, $"{nameof(Rumbles)}[{i}]: {nameof(Rumble)} may not be null.", out ex)
                || !ValidateNull(rumble.HeavyStates,
                    $"{nameof(Rumbles)}[{i}]: {nameof(Rumble.HeavyStates)} may not be null.", out ex)
                || !ValidateNull(rumble.LightStates,
                    $"{nameof(Rumbles)}[{i}]: {nameof(Rumble.LightStates)} may not be null.", out ex))
                return false;
        }
    ...
    public class Rumble
    {
        ...
        {
            var heavyCount = br.ReadInt16();
            var lightCount = br.ReadInt16();
            br.AssertInt32(0);
            var heavyOffset = br.ReadInt32();
            var lightOffset = br.ReadInt32();
