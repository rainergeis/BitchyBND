    ...
    public override bool Validate(out Exception ex)
    {
        ...
        for (var i = 0; i < Rooms.Count; i++)
        {
            ...
            if (!ValidateNull(room, $"{nameof(Rooms)}[{i}]: {nameof(Room)} may not be null.", out ex)
                || !ValidateNull(room.ConnectedRoomIndices,
                    $"{nameof(Rooms)}[{i}]: {nameof(Room.ConnectedRoomIndices)} may not be null.", out ex))
                return false;

            for (var j = 0; j < room.ConnectedRoomIndices.Count; j++)
            {
                var roomIndex = room.ConnectedRoomIndices[j];
                if (!ValidateIndex(Rooms.Count, roomIndex,
                        $"{nameof(Rooms)}[{i}].{nameof(Room.ConnectedRoomIndices)}[{j}]: Index out of range: {roomIndex}",
                        out ex))
                    return false;
            }
        }
