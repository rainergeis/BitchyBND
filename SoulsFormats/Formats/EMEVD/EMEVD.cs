...
public partial class EMEVD : SoulsFile<EMEVD>
{
    ...
    protected override void Read(BinaryReaderEx br)
    {
        ...
        var bigEndian = br.ReadBoolean();
        var is64Bit = br.AssertSByte(0, -1) == -1;
        var unk06 = br.ReadBoolean();
        var unk07 = br.AssertSByte(0, -1) == -1;
        br.BigEndian = bigEndian;
        br.VarintLong = is64Bit;

        var version = br.AssertInt32(0xCC, 0xCD);
        ...
        if (!bigEndian && !is64Bit && !unk06 && !unk07 && version == 0xCC)
            ...
            throw new NotSupportedException(
                $"Unknown EMEVD format: BigEndian={bigEndian} Is64Bit={is64Bit} Unicode={unk06} Unk07={unk07} Version=0x{version:X}");

        Offsets offsets;
        var eventCount = br.ReadVarint();
    ...
    {
        var bigEndian = Format == Game.DarkSouls1BE;
        var is64Bit = Format >= Game.Bloodborne;
        var unk06 = Format >= Game.DarkSouls3;
        var unk07 = Format >= Game.Sekiro;
        var version = Format < Game.DarkSouls3 ? 0xCC : 0xCD;
        ...
        {
            Event evt = Events[i];
            for (var j = 0; j < evt.Instructions.Count; j++)
        ...
        foreach (var offset in LinkedFileOffsets)
