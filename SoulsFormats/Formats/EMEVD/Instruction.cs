...
public partial class EMEVD
{
    ...
    public class Instruction
    {
        ...
        internal Instruction(BinaryReaderEx br, Game format, Offsets offsets)
        {
            Bank = br.ReadInt32();
            ID = br.ReadInt32();
            var argsLength = br.ReadVarint();
            var argsOffset = br.ReadVarint();
            long layerOffset;
        ...
        {
            var argsOffset = ArgData.Length > 0 ? bw.Position - offsets.Arguments : -1;
        ...
        internal void FillLayerOffset(BinaryWriterEx bw, Game format, int eventIndex, int instrIndex,
            Dictionary<uint, long> layerOffsets)
        {
            var layerOffset = Layer.HasValue ? layerOffsets[Layer.Value] : -1;
            if (format < Game.DarkSouls3)
