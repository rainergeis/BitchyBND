...
public class NVA : SoulsFile<NVA>
{
    ...
    public class Navmesh
    {
        ...
        internal Navmesh(BinaryReaderEx br, int version)
        {
            ...
            if (version < 4)
            {
                ...
                for (var i = 0; i < 16 - nameRefCount; i++)
                    br.AssertInt32(-1);
            }
            else
            {
                var nameRefOffset = br.ReadInt32();
                br.AssertInt32(0);
                br.AssertInt32(0);
                br.AssertInt32(0);
        ...
        internal void TakeMapNodes(MapNodeSection entries8)
        {
            ...
            for (var i = 0; i < MapNodeCount; i++)
                MapNodes.Add(entries8[MapNodesIndex + i]);
            MapNodeCount = -1;

            foreach (MapNode mapNode in MapNodes)
                if (mapNode.SiblingDistances.Count > MapNodes.Count)
                    mapNode.SiblingDistances.RemoveRange(MapNodes.Count,
                        mapNode.SiblingDistances.Count - MapNodes.Count);
        }
        ...
        internal void Write(BinaryWriterEx bw, int version, int index)
        {
            ...
            if (version < 4)
            {
                ...
                for (var i = 0; i < 16 - NameReferenceIDs.Count; i++)
                    bw.WriteInt32(-1);
            }
            else
            {
    ...
    public class Entry2
    {
        ...
        internal Entry2(BinaryReaderEx br)
        {
            ...
            for (var i = 0; i < 64 - referenceCount; i++)
                br.AssertInt64(0);
        }
        ...
        internal void Write(BinaryWriterEx bw)
        {
            ...
            for (var i = 0; i < 64 - References.Count; i++)
                bw.WriteInt64(0);
        }
    ...
    public class MapNode
    {
        ...
        internal MapNode(BinaryReaderEx br, int version)
        {
            ...
            {
                var subIDCount = br.ReadInt32();
                Unk14 = br.ReadInt32();
                var subIDsOffset = br.ReadInt32();
                br.AssertInt32(0);
                SiblingDistances = new List<float>(
        ...
        internal void Write(BinaryWriterEx bw, int version, int index)
        {
            ...
            if (version < 2)
            {
                ...
                foreach (var distance in SiblingDistances)
                    bw.WriteUInt16((ushort)(distance == -1 ? 0xFFFF : Math.Round(distance * 100)));

                for (var i = 0; i < 16 - SiblingDistances.Count; i++)
