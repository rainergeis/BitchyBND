...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member
    public class Mesh : IFlverMesh
    {
        ...
        internal Mesh(BinaryReaderEx br, FLVER0 flv, int dataOffset)
        {
            ...
            var vertexIndexCount = br.ReadInt32();
            ...
            var bufferDataLength = br.ReadInt32();
            var bufferDataOffset = br.ReadInt32();
            var vertexBuffersOffset1 = br.ReadInt32();
            var vertexBuffersOffset2 = br.ReadInt32();
            br.AssertInt32(0);

            if (flv.VertexIndexSize == 16)
            {
                VertexIndices = new List<int>(vertexCount);
                foreach (var index in br.GetUInt16s(dataOffset + vertexIndicesOffset, vertexIndexCount))
            ...
            else
            {
                br.StepIn(vertexBuffersOffset1);
                {
                    ...
                    for (var i = 1; i < vertexBuffers1.Count; i++)
                    ...
                    else
                    {
                        if (vi1 != vi2 && vi1 != vi3 && vi2 != vi3)
                        {
                            ...
                            if (checkFlip)
                            {
                                ...
                                var faceNormal = Vector3.Normalize(Vector3.Cross(v2.Position - v1.Position,
                                    v3.Position - v1.Position));
                                var angle = Vector3.Dot(faceNormal, vertexNormal) /
                                            (faceNormal.Length() * vertexNormal.Length());
                                flip = angle >= 0;
                                checkFlip = false;
                            }
