...
public static partial class FLVER
{
    ...
    public class Vertex
    {
        ...
        /// <summary>
        ///     Fourth component of the normal, read without transforming into a float; used as a bone index for binding to a
        ///     single bone.
        /// </summary>
        public int NormalW;
        ...
        {
            return (br.ReadByte() - 127) / 127f;
        }

        private static Vector3 ReadByteNormXYZ(BinaryReaderEx br)
        {
            return new Vector3(ReadByteNorm(br), ReadByteNorm(br), ReadByteNorm(br));
        }

        private static Vector4 ReadByteNormXYZW(BinaryReaderEx br)
        {
            return new Vector4(ReadByteNorm(br), ReadByteNorm(br), ReadByteNorm(br), ReadByteNorm(br));
        }

        private static float ReadSByteNorm(BinaryReaderEx br)
        {
            return br.ReadSByte() / 127f;
        }

        private static Vector3 ReadSByteNormZYX(BinaryReaderEx br)
        {
            var z = ReadSByteNorm(br);
            var y = ReadSByteNorm(br);
            var x = ReadSByteNorm(br);
            return new Vector3(x, y, z);
        }

        private static float ReadShortNorm(BinaryReaderEx br)
        {
            return br.ReadInt16() / 32767f;
        }

        private static Vector3 ReadShortNormXYZ(BinaryReaderEx br)
        {
            return new Vector3(ReadShortNorm(br), ReadShortNorm(br), ReadShortNorm(br));
        }

        private static Vector4 ReadShortNormXYZW(BinaryReaderEx br)
        {
            return new Vector4(ReadShortNorm(br), ReadShortNorm(br), ReadShortNorm(br), ReadShortNorm(br));
        }

        private static float ReadUShortNorm(BinaryReaderEx br)
        {
            return (br.ReadUInt16() - 32767) / 32767f;
        }

        private static Vector3 ReadUShortNormXYZ(BinaryReaderEx br)
        {
            return new Vector3(ReadUShortNorm(br), ReadUShortNorm(br), ReadUShortNorm(br));
        }
