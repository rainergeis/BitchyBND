...
public partial class PARAMDEF
{
    ...
    public class Field
    {
        ...
        private static readonly Regex arrayLengthRx = new(@"^\s*(?<name>.+?)\s*\[\s*(?<length>\d+)\s*\]\s*$");
        private static readonly Regex bitSizeRx = new(@"^\s*(?<name>.+?)\s*\:\s*(?<size>\d+)\s*$");
        ...
        internal Field(BinaryReaderEx br, PARAMDEF def)
        {
            ...
            var descriptionOffset = br.ReadVarint();
            ...
            if (def.FormatVersion >= 102)
            {
                ...
                if (ParamUtil.IsArrayType(DisplayType))
                {
                    ...
                    var length = match.Success ? int.Parse(match.Groups["length"].Value) : 1;
                    if (length != ArrayLength)
                        throw new InvalidDataException(
                            $"Mismatched array length in {InternalName} with byte count {byteCount}.");
                    if (match.Success)
                        InternalName = match.Groups["name"].Value;
                }
            }
            ...
            {
                br.AssertInt32(0);
                var unkB8Offset = br.ReadInt64();
                var unkC0Offset = br.ReadInt64();
                var unkC8Offset = br.ReadInt64();
        ...
        /// <summary>
        ///     Unknown; appears to be an identifier. May be null, only supported in versions >= 200, only present in version 202
        ///     so far.
        /// </summary>
        public string UnkB8 { get; set; }

        /// <summary>
        ///     Unknown; appears to be a param type. May be null, only supported in versions >= 200, only present in version 202 so
        ///     far.
        /// </summary>
        public string UnkC0 { get; set; }

        /// <summary>
        ///     Unknown; appears to be a display string. May be null, only supported in versions >= 200, only present in version
        ///     202 so far.
        /// </summary>
        ...
        internal void Write(BinaryWriterEx bw, PARAMDEF def, int index)
        {
            ...
            var padding = (byte)(def.FormatVersion >= 106 ? 0x00 : 0x20);
        ...
        internal void WriteStrings(BinaryWriterEx bw, PARAMDEF def, int index,
            Dictionary<string, long> sharedStringOffsets)
        {
            if (def.FormatVersion >= 202 || (def.FormatVersion >= 106 && def.FormatVersion < 200))
            {
