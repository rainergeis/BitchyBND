...
public partial class PARAMDEF
{
    private static class XmlSerializer
    {
        ...
        private static readonly Regex defOuterRx = new(@"^(?<type>\S+)\s+(?<name>.+?)(?:\s*=\s*(?<default>\S+))?$");
        private static readonly Regex defBitRx = new(@"^(?<name>.+?)\s*:\s*(?<size>\d+)$");
        private static readonly Regex defArrayRx = new(@"^(?<name>.+?)\s*\[\s*(?<length>\d+)\]$");
        ...
        public static PARAMDEF Deserialize(XmlDocument xml)
        {
            ...
            foreach (XmlNode node in root.SelectNodes("Fields/Field")) def.Fields.Add(DeserializeField(def, node));
        ...
        {
            var field = new Field();
            var fieldDef = node.Attributes["Def"].InnerText;
            ...
            var internalName = outerMatch.Groups["name"].Value.Trim();
            ...
            field.DisplayFormat =
                node.ReadStringOrDefault("DisplayFormat", ParamUtil.GetDefaultFormat(field.DisplayType));
            ...
            field.Minimum = ReadVariableValueOrDefault(def, node, field.DisplayType, "Minimum",
                ParamUtil.GetDefaultMinimum(def, field.DisplayType));
            field.Maximum = ReadVariableValueOrDefault(def, node, field.DisplayType, "Maximum",
                ParamUtil.GetDefaultMaximum(def, field.DisplayType));
            field.Increment = ReadVariableValueOrDefault(def, node, field.DisplayType, "Increment",
                ParamUtil.GetDefaultIncrement(def, field.DisplayType));
        ...
        private static object ReadVariableValueOrDefault(PARAMDEF def, XmlNode node, DefType type, string xpath,
            object defaultValue)
        {
            if (def.VariableEditorValueTypes)
                switch (type)
        ...
        {
            var fieldDef = $"{field.DisplayType} {field.InternalName}";
            ...
            xw.WriteDefaultElement("EditFlags", field.EditFlags.ToString(),
                ParamUtil.GetDefaultEditFlags(field.DisplayType).ToString());
            WriteVariableValue(def, xw, field.DisplayType, "Minimum", field.Minimum,
                ParamUtil.GetDefaultMinimum(def, field.DisplayType));
            WriteVariableValue(def, xw, field.DisplayType, "Maximum", field.Maximum,
                ParamUtil.GetDefaultMaximum(def, field.DisplayType));
            WriteVariableValue(def, xw, field.DisplayType, "Increment", field.Increment,
                ParamUtil.GetDefaultIncrement(def, field.DisplayType));
        ...
        private static void WriteVariableValue(PARAMDEF def, XmlWriter xw, DefType type, string localName, object value,
            object defaultValue)
        {
            if (def.VariableEditorValueTypes)
                switch (type)
                ...
                xw.WriteDefaultElement(localName, Convert.ToSingle(value), Convert.ToSingle(defaultValue), "R",
