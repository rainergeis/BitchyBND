...
public partial class PARAMDEF : SoulsFile<PARAMDEF>
{
    ...
    protected override void Read(BinaryReaderEx br)
    {
        ...
        var headerSize = br.AssertInt16(0x30, 0xFF);
        DataVersion = br.ReadInt16();
        var fieldCount = br.ReadInt16();
        var fieldSize = br.AssertInt16(0x48, 0x68, 0x6C, 0x88, 0x8C, 0xAC, 0xB0, 0xD0);
    ...
    {
        if (!(FormatVersion == 101 || FormatVersion == 102 || FormatVersion == 103 || FormatVersion == 104 ||
              FormatVersion == 106
        ...
        for (var i = 0; i < Fields.Count; i++)
        {
            ...
            if (!ValidateNull(field, $"{which}: {nameof(Field)} may not be null.", out ex)
                ...
                || !ValidateNull(field.DisplayFormat, $"{which}: {nameof(Field.DisplayFormat)} may not be null.",
                    out ex)
                || !ValidateNull(field.InternalType, $"{which}: {nameof(Field.InternalType)} may not be null.", out ex)
                || (FormatVersion >= 102 && !ValidateNull(field.InternalName,
                    $"{which}: {nameof(Field.InternalName)} may not be null on version {FormatVersion}.", out ex)))
                return false;
        }
    ...
    protected override void Write(BinaryWriterEx bw)
    {
        ...
        var fieldStringsStart = bw.Position;
        ...
        {
            var fieldStringsLength = bw.Position - fieldStringsStart;
