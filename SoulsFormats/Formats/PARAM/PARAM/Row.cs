    ...
    public class Row
    {
        ...
        public Row(int id, string name, PARAMDEF paramdef)
        {
            ...
            for (var i = 0; i < paramdef.Fields.Count; i++)
            {
                ...
                var value = ParamUtil.ConvertDefaultValue(field);
        ...
        internal void ReadCells(BinaryReaderEx br, PARAMDEF paramdef)
        {
            ...
            void checkOrphanedBits()
            {
                if (bitOffset != -1 && bitValue >> bitOffset != 0)
                    throw new InvalidDataException(
                        $"Invalid paramdef {paramdef.ParamType}; bits would be lost before +0x{br.Position - DataOffset:X} in row {ID}.");
            }
            ...
            for (var i = 0; i < paramdef.Fields.Count; i++)
            {
                ...
                else
                {
                    ...
                    var shifted = (bitValue << (BIT_VALUE_SIZE - field.BitSize - bitOffset)) >>
                                  (BIT_VALUE_SIZE - field.BitSize);
