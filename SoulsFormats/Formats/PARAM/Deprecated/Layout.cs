...
public partial class PARAM : SoulsFile<PARAM>
{
    ...
    public class Layout : List<Layout.Entry>
    {
        ...
        private Layout(XmlDocument xml)
        {
            ...
            {
                var enumName = node.Attributes["name"].InnerText;
        ...
        public PARAMDEF ToParamdef(string paramType, out List<PARAMTDF> paramtdfs)
        {
            ...
            foreach (Entry entry in this)
            {
                ...
                switch (entry.Type)
                {
                    case CellType.dummy8:
                        fieldType = PARAMDEF.DefType.dummy8;
                        break;
                    case CellType.b8:
                    case CellType.u8:
                    case CellType.x8:
                        fieldType = PARAMDEF.DefType.u8;
                        break;
                    case CellType.s8:
                        fieldType = PARAMDEF.DefType.s8;
                        break;
                    case CellType.b16:
                    case CellType.u16:
                    case CellType.x16:
                        fieldType = PARAMDEF.DefType.u16;
                        break;
                    case CellType.s16:
                        fieldType = PARAMDEF.DefType.s16;
                        break;
                    case CellType.b32:
                    case CellType.u32:
                    case CellType.x32:
                        fieldType = PARAMDEF.DefType.u32;
                        break;
                    case CellType.s32:
                        fieldType = PARAMDEF.DefType.s32;
                        break;
                    case CellType.f32:
                        fieldType = PARAMDEF.DefType.f32;
                        break;
                    case CellType.fixstr:
                        fieldType = PARAMDEF.DefType.fixstr;
                        break;
                    case CellType.fixstrW:
                        fieldType = PARAMDEF.DefType.fixstrW;
                        break;
        ...
        public static object ParseParamValue(CellType type, string value, CultureInfo culture)
        {
            ...
            if (type == CellType.s16)
                return short.Parse(value);
            if (type == CellType.u16)
                return ushort.Parse(value);
            if (type == CellType.x16)
                return Convert.ToUInt16(value, 16);
            if (type == CellType.s32)
                return int.Parse(value);
            if (type == CellType.u32)
                return uint.Parse(value);
            if (type == CellType.x32)
        ...
        public class Entry
        {
            ...
            public int Size
            {
                get
                {
                    if (IsVariableSize)
                        return size;
                    if (Type == CellType.s8 || Type == CellType.u8 || Type == CellType.x8)
                        return 1;
                    if (Type == CellType.s16 || Type == CellType.u16 || Type == CellType.x16)
                        return 2;
                    if (Type == CellType.s32 || Type == CellType.u32 || Type == CellType.x32 || Type == CellType.f32)
                        return 4;
                    // Not meaningful
                    if (Type == CellType.b8 || Type == CellType.b16 || Type == CellType.b32)
                        return 0;
            ...
            public bool IsVariableSize =>
                Type == CellType.fixstr || Type == CellType.fixstrW || Type == CellType.dummy8;
