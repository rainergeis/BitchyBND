...
internal static class XmlWriterExtensions
{
    ...
    {
        if (!Equals(value, defaultValue)) xw.WriteElementString(localName, value.ToString());
    }
    ...
    {
        if (value != defaultValue) xw.WriteElementString(localName, value.ToString(provider));
    }

    public static void WriteDefaultElement(this XmlWriter xw,
        string localName, float value, float defaultValue, string format, IFormatProvider provider)
    {
        if (value != defaultValue) xw.WriteElementString(localName, value.ToString(format, provider));
    }
}