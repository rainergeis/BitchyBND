...
public class BinaryReaderEx
{
    ...
    {
        var relPos = Stream.Position - start;
        if (relPos % align > 0)
    ...
    {
        var read = br.Read(buffer, index, count);
        if (read != count)
            throw new EndOfStreamException("Remaining size of stream was smaller than requested number of bytes.");
    }
    ...
    {
        var s = ReadASCII(values[0].Length);
        var valid = false;
    ...
    {
        StepIn(offset);
        var result = ReadShiftJIS(length);
        StepOut();
        return result;
    }
